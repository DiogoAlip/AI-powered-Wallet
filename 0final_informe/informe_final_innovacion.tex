...
La interfaz de usuario de FinancIA! fue diseñada con un enfoque responsivo (adaptable de escritorio a móvil) centrado en reducir el esfuerzo cognitivo del usuario mediante una jerarquía visual clara y un esquema de colores armónico. El diseño se divide en tres componentes visuales principales:
\begin{enumerate}
    \item \textbf{Panel Lateral (Sidebar / Drawer):} En dispositivos de escritorio, se mantiene fijo a la izquierda utilizando un tema oscuro que contrasta con la zona principal. En dispositivos móviles, se convierte en un menú lateral deslizable (Drawer) accesible mediante un ícono de hamburguesa. Este panel contiene:
    \begin{itemize}
        \item \textbf{Botón de Acción Principal (CTA):} Acceso rápido para añadir transacciones manualmente.
        \item \textbf{Sección Analytics:} Tarjetas de métricas clave que resumen los gastos totales, el promedio por transacción y la cantidad total de transacciones realizadas, permitiendo una lectura financiera rápida y acumulada.
        \item \textbf{Sección Filters:} Menú desplegable para filtrar los datos mostrados según períodos temporales específicos (ej: "Este mes").
    \end{itemize}
    \item \textbf{Vista Principal (Dashboard):} Un área de trabajo limpia de tema claro y minimalista que muestra los balances generales y las metas de ahorro. Si el usuario no ha registrado transacciones, se presenta un estado vacío (\textit{Empty State}) con un ícono ilustrativo y el mensaje de guía ``No expenses yet'', orientando al usuario hacia la primera acción.
    \item \textbf{Barra de Captura Conversacional y Multimodal (Bottom Bar):} Situada en la parte inferior de la pantalla de chat, es el núcleo interactivo con la IA. Permite la digitación de texto libre, e integra un botón de dictado de voz (micrófono) y un botón de adjuntos (clip de papel) que representa la funcionalidad de OCR de imágenes de recibos.
...
\begin{enumerate}
    \item \textbf{Inicialización y Onboarding:} El usuario accede a la plataforma y visualiza el Dashboard inicial. Si es un usuario nuevo, el sistema presenta un Dashboard con estados vacíos en transacciones y presupuestos que dirigen la atención visual a la interacción principal (el chatbot o el botón manual).
    \item \textbf{Ingreso de Datos Multimodal:} El usuario introduce el gasto mediante el método más cómodo según su contexto de movilidad:
    \begin{itemize}
        \item \textbf{Texto:} Escribe un mensaje corto en lenguaje natural (ej: \textit{``Gasté \$12 en comida fuera en Green Cafe''}).
        \item \textbf{Voz:} Activa el micrófono y dicta la transacción en voz alta.
        \item \textbf{Imagen:} Captura una foto de una boleta de compra.
    \end{itemize}
    \item \textbf{Procesamiento Inteligente y Orquestación:} La aplicación transmite el input al servidor backend, donde el motor de Gemini procesa la información mediante \textit{Function Calling} para extraer los parámetros estructurados (monto, comercio, categoría, tipo) y actualiza de inmediato la base de datos relacional SQLite.
    \item \textbf{Actualización Reactiva de la Interfaz:} El backend responde con el nuevo estado del usuario. El frontend actualiza de inmediato el almacenamiento de Zustand, reemplazando el \textit{Empty State} por el historial de transacciones y recalculando de manera dinámica las tarjetas de Analytics, los límites de presupuesto con alerta semafórica y la barra de progreso de metas de ahorro en un solo ciclo de renderizado.
\end{enumerate}
...
\label{tab:endpoints}
\begin{tabular}{p{1.2cm}p{4.7cm}p{3.8cm}p{4.8cm}}
\toprule
...
\addlinespace
POST & /api/finances/transactions & \{merchant, category, amount, type\} & \{success: true, transaction, state\} \\
\addlinespace
DELETE & /api/finances/transactions/:id & ID de Transacción & \{success: true, state\} \\
\addlinespace
PUT & /api/finances/budgets & \{category, limit, spent\} & \{success: true, state\} \\
\addlinespace
POST & /api/finances/savings/deposit & \{amount, note\} & \{success: true, state\} \\
\addlinespace
POST & /api/finances/savings/apply-recommendation & Ninguno & \{success: true, state\} \\
\addlinespace
